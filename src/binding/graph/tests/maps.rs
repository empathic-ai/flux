...
fn map_builders_render_keys_values_and_replace_nested_snapshots() {
    ...
    app.world_mut()
        .run_system_once(move |mut commands: Commands| {
            commands
                .entity(owners[0])
                .builder()
                .bind_map(source, "MapSource", "entries", render);
            commands
                .entity(owners[1])
                .builder()
                .bind_map_from(path!(source, MapSource.entries), render);
            commands.entity(owners[2]).builder().bind_map_from(
                process(
                    path!(source, MapSource.entries),
                    |map: HashMap<String, Vec<i32>>| Ok(map),
                ),
                render,
            );
            let mut graph = BindingGraph::new();
            let node = graph
                .source(path!(source, MapSource.entries).unwrap())
                .unwrap();
            commands
                .entity(owners[3])
                .builder()
                .bind_map_node(graph, node, render)
                .unwrap();
        })
        .unwrap();
    ...
    for expected in [
        ...
        for owner in owners {
            ...
            assert_eq!(
                HashMap::<String, Vec<i32>>::from_reflect(&view.value),
                Some(expected.clone())
            );
            let children: Vec<_> = app
                ...
                .unwrap_or_default();
            let actual: HashMap<_, _> = children
                .iter()
                .map(|child| {
                    let seen = app.world().get::<Seen>(*child).unwrap();
                    let nested: Vec<_> = app
                        .world()
                        .get::<Children>(*child)
                        .unwrap()
                        .iter()
...
fn map_view_round_trips_and_unconfigured_views_do_not_panic() {
    ...
    let view = ReactiveMapView {
        value: expected.to_dynamic_map(),
    ...
    for copy in [cloned, decoded, binary] {
        assert_eq!(
            HashMap::<String, String>::from_reflect(&copy.value),
            Some(expected.clone())
        );
        ...
        .run_system_once(move |mut commands: Commands| {
            assert!(
                commands
                    .entity(owner)
                    .builder()
                    .try_bind_map_from(Err::<BindingPath, _>(anyhow!("bad source")), render)
                    .is_err()
            );
        })
        .unwrap();
...
fn removing_one_map_entry_preserves_other_editors_without_rendering_them_again() {
    ...
    app.world_mut()
        ...
        .unwrap();
    app.update();
    let rows: HashMap<String, Entity> = app
        .world()
        .get::<Children>(owner)
        .unwrap()
        .iter()
    ...
    assert_eq!(
        app.world().get::<Draft>(editor).unwrap().0,
        "unsaved credentials"
    );
